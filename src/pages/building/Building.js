...
const BuildingOuterWrapper = styled.div`
    display: flex;
`;
...
const Building = () => {
  ...
  const [energyPerformanceGroupBy, setEnergyPerformanceGroupBy] = useState(
      'month');
  ...
  const [generalBuildingInformation, setGeneralBuildingInformation] = useState(
      null);
  ...
  const getBuildingInfo = async () => {
    ...
    if (!isInValid) {
      setIsLoading(true);
      const tmp = await getBuildingById(id,
          moment(startTime).format('YYYY-MM-DD'),
          moment(endTime).format('YYYY-MM-DD'),
          idToken);

      console.log('building info', tmp);
                  ...
                  <BuildingOuterWrapper>
                    ...
                    <BuildingWrapper>
                      <BuildingInfo
                          ...
                          address={generalBuildingInformation.prop.streetAddress +
                              ', ' + generalBuildingInformation.prop.city +
                              ', ' +
                              findCountryByCountryCode(
                                  generalBuildingInformation.prop.countryCode)?.name +
                              ', ' +
                          ...
                          constructed={generalBuildingInformation.prop.completionYear +
                              ' - ' +
                              (generalBuildingInformation.prop.completionYear +
                                  10)}
                      ...
                      {isDisplayPerformanceFilter && (
                          <FilterWrapper
                              className="my-5 d-block justify-content-between ">
                            <GroupBy
                                className="d-flex justify-content-start align-content-end mb-2">
                              <ChartType type={groupBy}
                                         onChange={handleGroupByChange}/>

                              <div>{isInValid && <ErrorMsg>{t(
                                  'Start date should be greater than End date')}</ErrorMsg>}</div>
                            </GroupBy>
                      ...
                      <Routes>
                        ...
                        <Route path="asset-reliability/*"
                               element={<AssetReliability
                                   data={generalBuildingInformation.energyPerformance}/>}/>
                        <Route path="weather/*"
                               element={<Weather
                                   buildingGeneralInfo={generalBuildingInformation?.prop}
                        ...
                        <Route path="/"
                               element={<Navigate to="energy-performance"
                                                  replace/>}/>
                      </Routes>
